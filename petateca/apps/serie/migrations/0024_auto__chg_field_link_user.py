...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Renaming column for 'Link.user' to match new field type.
        db.rename_column('serie_link', 'user', 'user_id')
        # Changing field 'Link.user'
        db.alter_column('serie_link', 'user_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['auth.User']))

        # Adding index on 'Link', fields ['user']
        db.create_index('serie_link', ['user_id'])


    def backwards(self, orm):
        
        # Removing index on 'Link', fields ['user']
        db.delete_index('serie_link', ['user_id'])

        # Renaming column for 'Link.user' to match new field type.
        db.rename_column('serie_link', 'user_id', 'user')
        # Changing field 'Link.user'
        db.alter_column('serie_link', 'user', self.gf('django.db.models.fields.CharField')(max_length=255, null=True))
