...
def series_list(request):
    ...
    for serie in series:
        ...
        series_list['id'] =  serie.id
        all_series.append(series_list)
    return HttpResponse(simplejson.dumps(all_series, indent=4), mimetype='application/json')
...
def serie_detail(request, id_serie):
    ...
    serie_info['url'] = urlprefix + serie.get_absolute_url()
    ...
    return HttpResponse(simplejson.dumps(serie_info, indent=4), mimetype='application/json')


def season_detail(request, id_serie, season):
    ''' Devuelve el listado de episodios con id '''
    season = Season.objects.get(serie=Serie.objects.get(id=id_serie), season=season)
    epi_list = []
    for e in season.episodes.all():
        ...
        epi['url'] = urlprefix + e.get_absolute_url()
        epi_list.append(epi)
    return HttpResponse(simplejson.dumps(epi_list, indent=4), mimetype='application/json')
...
def episode_detail(request, id_serie, season, episode):
    ...
    return HttpResponse(simplejson.dumps(epi_detail, indent=4), mimetype='application/json')
