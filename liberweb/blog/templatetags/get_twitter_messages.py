    ...
    def render(self, context):
        try:
            ...
            most_recent_messages = [{"text":s.text, 
                                     ...
                                     "url": "http://twitter.com/%s/statuses/%s" % (self.user, s.id),} for s in status_obj][:self.limit]
...
def twitter_status(parser, token):
    ...
    if bits[1] != "user":
        raise template.TemplateSyntaxError, "First argument for %s should be 'user'" % bits[0]
    
    if bits[3] != "limit":
        raise template.TemplateSyntaxError, "Second argument for %s should be 'limit'" % bits[0]
    
    if bits[5] != "as":
        raise template.TemplateSyntaxError, "Third argument for %s should be 'as'" % bits[0]
